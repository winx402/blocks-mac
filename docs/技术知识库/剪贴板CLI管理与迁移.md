...
Beta 8 起，带自定义标签或已有分组归属的条目默认不参与自动时间/条数清理，取消收藏不会解除这项保护；手动明确删除仍可执行。受保护条目占用总条数预算，超过上限时仍保留全部受保护项。导入提示改为 `unprotected_imports_follow_normal_retention`。新建标签默认选取首个未用调色板颜色，用尽后轮换，CLI 与设置页一致；已有颜色不批量迁移。
